...
- ✅ **AiVante Professional Theme (4th Theme)** — Added 4th switchable theme based on aivante.com brand palette. Primary: steel blue `#3F5364`, secondary/accent: orange `#FD7800`, body font: Open Sans, heading font: Roboto Condensed. Introduced per-theme font switching via `--app-font-body` / `--app-font-heading` CSS custom properties. Changes across 4 files: `index.html` (Google Fonts), `styles.scss` (color + font tokens), `material-theme.scss` (M3 theme + typography overrides), `theme.service.ts` (AppTheme union + metadata).
- ✅ **HTTP Subscription Storm Audit & Fixes** — Codebase-wide audit identified and fixed 6 unguarded HTTP subscription patterns across 5 files (plan-recommendation autosave, user-profile MAGI/ZIP lookups, dashboard SignalR hydration, chat profile extraction, reference data loading). All fixes use cancel-before-fire `Subscription` pattern or in-flight boolean guards. Prior fix also added MAGI tier response caching in `CountyLookupService` and dedup fields in `ChatComponent`.
