...
The application is multi-tenant via a four-role hierarchy — **admin** seeds **Financial Planner Groups (FPGs)**, FPGs onboard **Financial Planners (FPs)**, and FPs create end-users and walk through Medicare wizards on their behalf via short-lived JWT impersonation. Public sign-up is disabled. See [ADMIN_SETUP.md](ADMIN_SETUP.md) for bootstrap credentials and the cascade flow.
...
| 5  | [Data & Authentication](ch05-data-and-auth/ch05-data-and-auth.md) | MongoDB database schema, JWT authentication, role-based access (admin / FPG / FP / user) and impersonation | `ch05-data-and-auth/` |
| 6  | [API Contract](ch06-api-contract/ch06-api-contract.md) | Endpoints, request/response schemas, and examples — *7 sub-files, 55 endpoints* | `ch06-api-contract/` |
