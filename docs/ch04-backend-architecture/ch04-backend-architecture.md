...
- **ImpersonationLoggingMiddleware:** Pushes `ImpersonatedBy={fpUserId}` into the Serilog `LogContext` for the duration of any request whose principal carries an `actingAs` claim, so every log line emitted while impersonating records the FP's id. Registered after `UseAuthentication()` (the principal must be populated) and before `UseAuthorization()`.

### Filters
- **MustChangePasswordFilter** (`Filters/MustChangePasswordFilter.cs`): Global `IAsyncActionFilter` registered via `AddControllers(options => options.Filters.Add<MustChangePasswordFilter>())`. Throws `UnauthorizedException` for any authenticated request other than `POST /api/auth/change-password` while the principal's `mustChangePassword` claim is `"true"`. Anonymous endpoints pass through untouched.
...
|------------|------|---------|
| `DrugController` | `[Authorize]` | `POST api/drug/suggest-names` — accepts `DrugNameRequest`, calls `IDrugAiService.SuggestDrugNames()`, returns `DrugNameSuggestionResult` with candidate names. |
| `PharmacyController` | `[Authorize]` | `GET api/pharmacy/lookup?page=&size=&radius=&name=` — Financial Planner pharmacy lookup using user profile lat/lng, returns paginated `PharmacyLookupResponse` with distance, address, zipcode. |
| `PlanRecommendationController` | `[Authorize]` | `POST api/plan-recommendation/evaluate-costs` — calculate costs + AI evaluation returning chart-ready `CostProjectionResult` (yearly details, lifetime totals, AI evaluation with categories, savings tips, trajectory). Delegates to `CostProjectionService`. |
| `AuthController` | Public (+1 Authorize) | signup, signin, forgot-password, reset-password, verify-email, resend-verification, change-password |
| `ReferenceDataController` | Public | `GET api/reference-data` — all master data for profile form dropdowns (static/in-memory). |
| `ProfileController` | `[Authorize]` | Consolidated profile CRUD. `GET api/profile` returns `UserProfileResponse` (joined view across login + profile docs). `POST api/profile` saves/updates `ProfileDto` (names go to `users`, everything else to `userProfiles`). API contract is unchanged by the split. |
| `PrescriptionController` | `[Authorize]` | `POST api/prescription/current` — saves current analysis selections. `PUT api/prescription/current/drugs` — updates drugs. `PUT api/prescription/current/pharmacy` — updates pharmacy. `PUT api/prescription/current/plans` — updates plans. `GET api/prescription/{id}` — returns a single prescription by ID. |
...
| `FinancialPlannerDrugController` | `[Authorize]` | `POST api/FinancialPlannerDrug/search-bulk` — accepts `BulkDrugSearchInput` with `List<string> DrugNames`, searches each drug, matches, fetches details, and if >1 drug calls AI to evaluate pairwise interactions and duplicate therapies. Returns `BulkDrugSearchResponse`. |
...
| `RecommendationController` | `[Authorize]` | Routes at `api/recommendation`. `GET` — returns active recommendation for user. `GET {id}` — returns recommendation by ID. `GET all` — list all recommendations (summary response with drug/plan counts, cost totals). `POST` — creates new recommendation (`CreateRecommendationRequest` with profile, drugs, pharmacies, plans, costSnapshot; `?force=true` replaces existing). `PUT profile` — updates profile snapshot. `PUT drugs` — replaces drug list. `PUT pharmacy` — updates pharmacy + mail order. `PUT plans` — replaces plan selections. `PUT cost-snapshot` — saves cost projection snapshot. `DELETE` — removes active recommendation. |
...
| `LtcSelectionsController` | `[Authorize]` | Routes at `api/ltc`. `PUT current` — accepts `SaveLtcCurrentRequest` (healthProfile, care-type year counts), upserts `LtcCurrentSelectionsDocument` in MongoDB. `GET current` — returns `LtcCurrentResponse` (care-type selections). |
...
| `AdminController` | `[Authorize(Roles = admin)]` | Routes at `api/admin`. Primary endpoints: `GET fpg-admin-users`, `POST fpg-admin-users`, `DELETE fpg-admin-users/{userId}`. The POST atomically auto-creates the underlying `FinancialPlannerGroup`; the DELETE refuses with `409` if the group still has FPs (top of the bottom-up tear-down chain — see [ch06-07](../ch06-api-contract/ch06-07-roles-impersonation-endpoints.md)). Legacy endpoints kept for back-compat: `GET/POST financial-planner-groups`, `POST financial-planner-groups/{fpgId}/admin-user`. |
| `FinancialPlannerGroupController` | `[Authorize(Roles = financial_planner_group)]` | Routes at `api/financial-planner-group`. Reads caller's `fpgId` claim. `GET me`, `GET/POST/PUT/DELETE me/financial-planners[/{fpUserId}]`, `GET me/end-users` (read-only across the group), `GET me/recommendations` (grouped by user, latest first). Delete-FP rejects with 409 if the FP has end-users. |
| `FinancialPlannerController` | `[Authorize(Roles = financial_planner)]` | Routes at `api/financial-planner`. Reads caller's user id from `NameIdentifier`. `GET/POST me/end-users`, `DELETE me/end-users/{endUserId}` (cascade delete across profile/chat/recs/analysis/LTC), `GET me/recommendations`, `DELETE me/recommendations/{id}`. Create-end-user accepts first/last/email/phone/password from the FP (every role's creation form now follows the same shape: identity + phone + password). Phone is normalized and checked against the unique-phone index. Sets `MustChangePassword = true`. |
| `ImpersonationController` | Class `[Authorize]`; action `[Authorize(Roles = financial_planner)]` on POST | Routes at `api/impersonate`. `POST` — FP starts impersonating; verifies `target.FpId == caller`. Issues 60-min JWT via `IJwtTokenIssuer` with `Role=user`, `actingAs=fpUserId`, `mustChangePassword=false` (overridden so the impersonated session isn't blocked by the user's first-login flag). `POST refresh` — accepts the impersonation token, reads `actingAs` claim, reissues fresh 60-min token. |
...
| File | Key Types |
|------|-----------|
| `Documents/UserDocument.cs` | `UserDocument` — login / identity document in the `users` collection (userId, email, phone, passwordHash, isEmailVerified, mustChangePassword, firstName, lastName, role, fpgId, fpId, audit). `[BsonIgnoreExtraElements]` so legacy unified docs deserialize cleanly during transition. |
| `Documents/ProfileDocument.cs` | `ProfileDocument` — personal / medical / address document in the `userProfiles` collection (userId FK, coverageYear, healthCondition, taxFilingStatus, magiTier, gender, tobaccoStatus, dateOfBirth, lifeExpectancy, concierge / conciergeAmount, alternateEmail / alternateMobile, address fields, currentPrescriptionDocumentId, isProfileComplete, audit). Linked to `UserDocument` by `userId`. |
| `Documents/ChatSessionDocument.cs` | Chat session messages + UI state |
| `Documents/LtcCurrentSelectionsDocument.cs` | `LtcCurrentSelectionsDocument` (userId, name, healthProfile, numberOfAdultDayHealthCareYears, numberOfHomeCareYears, numberOfNursingCareYears, createdAt, updatedAt) — one document per user in collection `ltcCurrentSelections` |
| `Documents/PrescriptionDocument.cs` | Named prescriptions with embedded drug list |
| `Documents/RecommendationDocument.cs` | `RecommendationDocument` (root — userId, name, status, profile snapshot, planSelections, drugList, pharmacies, mailOrderPharmacy, lastCostSnapshot, timestamps), `ProfileSnapshot` (full profile mirror: name, DOB, gender, zip, county, state, city, address, coverage year, health, LIS), `SelectedDrugDoc`, `SelectedPlanDoc` (+deductible, starRating, totalPrescriptionCost, totalPlanCost, prescriptionDrugCovered, unavailableDrugs[], planExpenses[]), `SelectedPharmacyDoc`, `MailOrderPharmacyDoc`, `CostSnapshotDoc` (+supplementPlanType, supplementPlanPremium, yearlyDetails[], evaluation), `YearlyDetailDoc` (20 per-year financial fields including planG/F/NPremium, totalABGD/ABFD/ABND/ABCD), `CostEvaluationDoc`, `LifetimeSummaryDoc`, `YearlyHighlightDoc`, `CostCategoryDoc`, `SavingsTipDoc`, `PlanExpenseDoc` |
...
- `IProfileRepository` (GetByUserIdAsync, CreateAsync, UpdateAsync, ExistsByUserIdAsync, DeleteByUserIdAsync — operates on `ProfileDocument` against the `userProfiles` collection).
- `IUserRepository` — login / identity queries (GetByEmail, GetByPhone, GetById, Create, Update, Delete, EmailExists, PhoneExists, plus the role-hierarchy queries GetByFpId / GetByFpgIdAndRole / GetEndUsersByFpg / GetAllByRoleAsync — operates on `UserDocument`). `GetAllByRoleAsync` is admin-scope (used to list every FPG admin without an `fpgId` filter).
...
| Service | Role |
|---------|------|
| `ProfileService` | Consolidated profile CRUD. Reads `UserDocument` via `IUserRepository` (for `FirstName` / `LastName`) and `ProfileDocument` via `IProfileRepository` (for everything else). `SaveAsync` writes names to the login doc and the rest to the profile doc, lazily creating the `ProfileDocument` on first save. Maps (`UserDocument` + `ProfileDocument`) → `ProfileDto`. |
| `AuthService` (impl. `IAuthService`) | JWT auth logic — signin, forgot/reset/verify/resend, change-password (clears MustChangePassword + reissues fresh JWT). Token issuance delegated to `IJwtTokenIssuer`. **Sign-up was removed** — accounts are created top-down via Admin / FPG / FP services. |
| `JwtTokenIssuer` (impl. `IJwtTokenIssuer`) | Single source of truth for JWT issuance. Builds claims (`NameIdentifier`, `Email`, `Role`, `mustChangePassword`, `Jti`, plus `fpgId`/`fpId`/`actingAs` when applicable), signs with HMAC-SHA256, configurable lifetime (default 24h, 60-min override for impersonation). Used by `AuthService` and `ImpersonationService`. |
| `AdminService` (impl. `IAdminService`) | Primary methods: `ListFpgAdminUsersAsync` (all users with role `financial_planner_group`, newest first), `CreateFpgAdminUserAsync` (validates email + phone uniqueness up front so we don't orphan the auto-created group; phone normalized via `PhoneNormalizer.NormalizeUsPhone`; auto-creates a `FinancialPlannerGroup` named `"{FirstName} {LastName}"` with `" 2"`, `" 3"`, … suffix on collision, fails after 50; user gets `MustChangePassword = true`), and `DeleteFpgAdminUserAsync` (deletes the user + their auto-created group; refuses with `ConflictException` if the group still has FPs). Legacy methods `ListGroupsAsync` / `CreateGroupAsync` / `CreateGroupAdminUserAsync` kept for back-compat (now also use the supplied phone + uniqueness check). |
| `FinancialPlannerGroupService` (impl. `IFinancialPlannerGroupService`) | FP CRUD scoped to caller's `fpgId` claim. Re-fetches and verifies `target.FpgId == callerFpgId` on every mutation. Delete rejected with 409 if the FP still has end-users. Group views (`ListGroupEndUsersAsync`, `ListGroupRecommendationsAsync`) join through `IUserRepository.GetEndUsersByFpgAsync` + `IRecommendationRepository.GetByUserIdsAsync`. |
| `FinancialPlannerService` (impl. `IFinancialPlannerService`) | End-user list (`WHERE FpId = caller`), recommendations grouped by user (latest first), delete recommendation (verifies rec.user.FpId == caller before deleting), **cascade-delete end-user** (`DeleteEndUserAsync` — wipes the user's profile, chat session, recommendations, analysis selections, LTC selections, then the user itself; verifies caller owns the target). |
| `EndUserService` (impl. `IEndUserService`) | Creates an end-user from `CreateEndUserRequest` (first/last/email/phone/password). Email lowercased and uniqueness-checked. Phone normalized via `PhoneNormalizer.NormalizeUsPhone` (10-digit string) and checked against the unique-phone index — duplicate returns `ConflictException`. Password BCrypt-hashed. Sets `MustChangePassword = true`, `IsEmailVerified = true`, `FpId = callerFpUserId`. |
| `ImpersonationService` (impl. `IImpersonationService`) | Verifies caller is FP and target is one of their end-users (`target.Role == user && target.FpId == fpUserId`). Clones the target with `MustChangePassword = false` and calls `IJwtTokenIssuer.Issue` with `actingAs = fpUserId` and a 60-min TTL. Logs FP + target email + expiry. |
...
| Class | Role |
|-------|------|
| `MongoUserRepository` | Login / identity queries against MongoDB `users` collection. Implements `IUserRepository`. |
| `MongoProfileRepository` | Profile queries against MongoDB `userProfiles` collection. Implements `IProfileRepository`. `UpdateAsync` is an upsert so the profile document is created lazily on first save. |
| `MongoDbContext` | MongoDB typed collection accessor. Provides `Users`, `UserProfiles`, `UsersRaw` (used by the split migration), `Prescriptions`, `ChatSessions`, `UserAnalysisSelections`, `Recommendations`, `LtcCurrentSelections`, `FinancialPlannerGroups` collections. |
| `MongoIndexInitializer` | `IHostedService` — creates compound indexes at startup (unique sparse Email/Phone/UserId on `users`, unique UserId on `userProfiles`, **unique sparse UserId on `chatSessions`**, unique (UserId, Name) on `userAnalysisSelections` and `ltcCurrentSelections`, (UserId, Status) + CreatedAt on `recommendations`, unique GroupId/Name on `financialPlannerGroups`). Two cleanup helpers run before index creation: `DropLegacyPascalCaseIndexesAsync` drops indexes whose names start with an uppercase letter (stale from before the camelCase convention was registered); `DropOptionDriftedIndexesAsync` drops indexes whose options have drifted from the desired spec (currently: `chatSessions.userId_1` previously created without `sparse:true`). Both are idempotent and safe on every startup. |
| `UserProfileSplitMigrationInitializer` | `IHostedService` registered **before** `MongoIndexInitializer`. One-shot migration that scans the legacy unified `users` documents, upserts profile fields into a new `userProfiles` doc, then `$unset`s those fields on `users`. Idempotent via a marker doc in `schemaMigrations` — subsequent startups detect the marker and no-op. |
| `AdminSeedInitializer` | `IHostedService` registered after the index initializer. Seeds the singleton admin user when `Seed:AdminPassword` is configured (production-safe by default — no password means no seed). Email and phone are configurable via `Seed:AdminEmail` (default `admin@aivante.com`) and `Seed:AdminPhone` (default `5550199999`). In docker the three values map to `ADMIN_EMAIL` / `ADMIN_PHONE` / `ADMIN_PASSWORD` env vars (see [DOCKER.md](../DOCKER.md) and [ADMIN_SETUP.md](../ADMIN_SETUP.md)). |
