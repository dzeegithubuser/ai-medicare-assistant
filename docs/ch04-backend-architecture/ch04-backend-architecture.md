...
| `MongoDbContext` | MongoDB typed collection accessor. Provides `Users`, `Prescriptions`, `ChatSessions`, `UserAnalysisSelections`, `Recommendations`, `LtcCurrentSelections` collections. |
| `MongoIndexInitializer` | `IHostedService` — creates compound indexes at startup (including unique sparse Email, Phone, UserId on `users`, unique UserId on `chatSessions`, unique (UserId, Name) on `userAnalysisSelections` and `ltcCurrentSelections`, and (UserId, Status) + CreatedAt on `recommendations`). |
