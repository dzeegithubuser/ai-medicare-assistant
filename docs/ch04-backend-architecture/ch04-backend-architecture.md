...
| `FinancialPlannerController` | `[Authorize(Roles = financial_planner)]` | Routes at `api/financial-planner`. Reads caller's user id from `NameIdentifier`. `GET/POST me/end-users`, `DELETE me/end-users/{endUserId}` (cascade delete across profile/chat/recs/analysis/LTC), `GET me/recommendations`, `DELETE me/recommendations/{id}`. Create-end-user uses default password `Aivante@1234` and randomized `55501XXXXX` dummy phone, sets `MustChangePassword = true`. |
...
| `AdminService` (impl. `IAdminService`) | Primary methods: `ListFpgAdminUsersAsync` (all users with role `financial_planner_group`, newest first), `CreateFpgAdminUserAsync` (atomically auto-creates a `FinancialPlannerGroup` named `"{FirstName} {LastName}"` with `" 2"`, `" 3"`, … suffix on collision, fails after 50; user gets `MustChangePassword = true` and a randomized `55501XXXXX` dummy phone), and `DeleteFpgAdminUserAsync` (deletes the user + their auto-created group; refuses with `ConflictException` if the group still has FPs). Legacy methods `ListGroupsAsync` / `CreateGroupAsync` / `CreateGroupAdminUserAsync` kept for back-compat. |
...
| `EndUserService` (impl. `IEndUserService`) | Creates an end-user with default password **`Aivante@1234`** (constant), randomized `55501XXXXX` dummy phone, `MustChangePassword = true`, `IsEmailVerified = true`, `FpId = callerFpUserId`. Phone uniqueness retried up to 25 times. |
